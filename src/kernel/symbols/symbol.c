...
void symbol_init() {
	...
	paging_identity_map(header, header->e_ehsize);
	pageframe_reserve(header, NEAREST_PAGE(header->e_ehsize));
	...
	paging_identity_map(section_headers, section_headers_num * header->e_shentsize);
	pageframe_reserve(section_headers, NEAREST_PAGE(section_headers_num * header->e_shentsize));
	...
	for(size_t i = 0; i < section_headers_num; i++) {
		...
		if(section_header->sh_type == ELF_SECTION_HEADER_TYPE_SYMTAB) {
			...
			paging_identity_map(symbol_table, section_header->sh_size);
			pageframe_reserve(symbol_table, NEAREST_PAGE(section_header->sh_size));
			log("Symbol table located, offset: 0x%x\n", section_header->sh_offset);
			...
			paging_identity_map(string_table, string_table_size);
			pageframe_reserve(string_table, NEAREST_PAGE(string_table_size));
